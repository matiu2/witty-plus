...
class DBNoDupValidator : public WValidator {
public:
    typedef dbo::dbo_traits<ModelClass> Traits;
    //typedef dbo::dbo_default_traits Traits;
private:
    dbo::Session& _db;
    string _fieldName;
...
public:
    /** This constructor is for when you're editing new objects. It'll return Invalid if any object in the DB has the same name.
      ...
      **/
    DBNoDupValidator(dbo::Session& db, const std::string& fieldName, bool isMandatory=false, WObject* parent=0) :
        WValidator(isMandatory, parent), _db(db), _fieldName(fieldName), _query(DBNoDupValidator::makeQuery(db, fieldName)) {}
    /** This constructor is for when you are editing an existing object. You want there to be zero dups, but of course if
          ...
          @param parent the parent that owns this validator
    **/
    DBNoDupValidator(dbo::Session& db, const std::string& fieldName, typename Traits::IdType idToIgnore, bool isMandatory=false, WObject* parent=0) :
        WValidator(isMandatory, parent), _db(db), _fieldName(fieldName),
